...
class SigLIPInference:
    ...
    def predict_multi_label(self, image_paths, captions, image_batch_size=128, caption_batch_size=1024, threshold=0.95, k = 5, method="sigmoid"):
    ...
    def predict_single_label(self, image_paths, captions, image_batch_size=128, caption_batch_size=512, method='sigmoid'):
...
def ensemble_submissions(file_paths, tiebreaker_file_path, t1_len, t2_len):
    ...
    test2_labels = []
    for i in range(t1_len, len(dfs[0])):
...
def main():
    ...
    ensembled = ensemble_submissions(files_to_ensemble, tiebreaker_file, t1_len, t2_len)
